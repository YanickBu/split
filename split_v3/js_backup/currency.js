const Currency = {
  ...
  async fetchRates() {
    const now = Date.now();
    if (now - this.lastFetch < 3600000 && Object.keys(this.rates).length > 0)
      return true;
    ...
    try {
      ...
      if (data && data.rates) {
        ...
        localStorage.setItem(
          "split_rates",
          JSON.stringify({ rates: this.rates, ts: this.lastFetch }),
        );
        return true;
      }
    } catch (e) {
  ...
  async fetchHistoricalRates(dateStr) {
    ...
    try {
      ...
      console.warn(
        `Failed to fetch historical rates for ${dateStr} from Frankfurter, trying fallback...`,
        e,
      );
    }
    ...
    try {
      ...
      console.warn(
        `Historical rate fallback failed for ${dateStr}, using current rates`,
      );
    }
