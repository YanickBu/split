const CSVExport = {
  exportCSV(app, e) {
    ...
    const headers = ["ID", "Date", "Title", "Payer", "Split Members", "Original Amount", "Original Currency", "Group Amount", "Group Currency", "Status", "Exchange Rate"];
      ...
      reader.onload = async () => {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          if (!group.members.includes(payer)) {
            await State.appendEvent(App.currentGroupId, 'ADD_MEMBER', { name: payer });
          }
          ...
          if (splitMembersStr) {
            ...
            for (const member of splitMembers) {
              if (!group.members.includes(member)) {
                await State.appendEvent(App.currentGroupId, 'ADD_MEMBER', { name: member });
              }
            }
          }
        ...
        App.render();
        alert(`Imported ${importedCount} expenses.` + (errorsCount ? ` Skipped ${errorsCount} rows with errors.` : ''));
      };
